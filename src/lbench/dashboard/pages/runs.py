...
def benchmark_to_table(bm, run_name):
    ...
    # Get all groups from registry (ordered)
    all_groups = registry.list_groups()

    # Render each group's card
    for group in all_groups:
        card_body = group.render_card(bm, run_name)
        if card_body:
            card_children.append(card_body)

    # Collect action buttons from all groups
    buttons = []
    for group in all_groups:
        group_buttons = group.get_action_buttons(bm, run_name)
        buttons.extend(group_buttons)

    # Add buttons section if any buttons exist
    if buttons:
